...
from report.gather.content_report import ContentReport
from report.gather.metadata_item import MetadataItem
from report.gather.metadata_report import MetadataReport
...
class ReportService:
    ...
    def start(self, items_dir: Path, cache_dir: Path, output_dir: Path) -> None:
        """Recognise text from each line of each pdf page."""
    ...
    def process_item(self, item_file: Path):
        ...
        referrers = [
            ...
            + "Senate/Senators_Interests/Register46thparl",
        ]

        for item in Item.load(item_file):
            ...
            text_found = [list(Item.load(p)) for p in csv_files]
