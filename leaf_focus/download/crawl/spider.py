...
class Spider(ScrapySpider):
    ...
    def set_custom_start_urls(self):
        ...
        if not start_urls:
            raise ValueError("Custom start urls must be provided.")
    ...
    def set_custom_link_extractor(self):
        ...
        if not allowed_domains:
            raise ValueError("Custom allowed domains must be provided.")
    ...
    def _extract_info(self, response: Response, link: Link):
        ...
        templates = {
            "last_updated": [
                Template(
                    '//a[contains(@href, "${url}")]/parent::td/'
                    + 'parent::tr/td[@class="date"]/text()'
                ),
        ...
        is_member = (
            len(response.xpath('//text()[contains(.,"Members\' Interests")]')) > 0
        )
        is_senator = (
            len(response.xpath('//text()[contains(.,"Senators\' Interests")]')) > 0
        )
